    ...
    try {
        ...
        data.forEach((pilot, key) => {
            ...
            if (key === 0) {
                ...
                const dataSelect = `
                    <img title="${pilot.name}" src="./src/images/card-${pilot.cardName}.png" alt="Piloto ${pilot.name}" class="piloto-grande">
                    <div class="informacoes-piloto">
                        <h2 class="nome-piloto" id="nome-piloto">${pilot.name}</h2>
                        <p class="descricao-piloto" id="descricao-piloto">
                            ${pilot.description}
                        </p>
                    </div>
                `;
            ...
            imagePilot.setAttribute('title', pilot.name);
            imagePilot.setAttribute('alt', `Piloto ${pilot.name}`);
